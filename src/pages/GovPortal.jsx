...
// --- MOCK CONSTANTS ---
const USE_MOCK = true;
const ADMIN_CREDENTIALS = { username: "gov_admin", password: "Gov@1234" };
...
export default function GovPortal() {
  ...
  const handleLogin = (e) => {
    ...
    if (loginForm.username === ADMIN_CREDENTIALS.username && loginForm.password === ADMIN_CREDENTIALS.password) {
      setUser({ name: "Ministry of Education", role: "Super Admin" });
      fetchData();
    } else {
      setError("Invalid Credentials. Try gov_admin / Gov@1234");
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800">
       ...
       <aside className="w-64 bg-slate-900 text-white flex-shrink-0 hidden lg:flex flex-col sticky top-0 h-screen">
          ...
          <nav className="flex-1 p-4 space-y-1">
             {["Overview", "Institutes", "Students", "Placements", "Reports"].map(tab => (
                <button 
                  key={tab}
          ...
          <div className="p-6 border-t border-slate-800">
             ...
             <button onClick={() => setUser(null)} className="mt-3 text-xs text-red-400 hover:text-red-300">Sign Out</button>
          </div>
       </aside>
       ...
       <main className="flex-1 min-w-0">
          ...
          <header className="bg-white border-b border-slate-200 px-8 py-4 sticky top-0 z-20 flex justify-between items-center">
             ...
             <div className="flex gap-3">
                <button className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-bold text-slate-700 hover:bg-slate-50">Notifications</button>
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KPICard label="Total Enrolled Students" value={data.overview.totalStudents.toLocaleString()} trend="+4.5%" positive={true} />
        <KPICard label="Registered Institutes" value={data.overview.totalColleges} trend="+2 New" positive={true} />
        <KPICard label="Placement Rate" value={data.overview.placementRate + "%"} trend="+1.2%" positive={true} />
        <KPICard label="Average Package" value={data.overview.avgPackage} trend="+0.5 LPA" positive={true} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                  ...
                  <select className="text-xs border-slate-200 rounded-lg bg-slate-50"><option>National Average</option></select>
              </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
               ...
               <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                     {data.colleges.slice(0, 3).map((c) => (
                        <tr key={c.id} className="hover:bg-slate-50">
                           <td className="px-6 py-3 font-medium text-slate-900">{c.name}</td>
                           <td className="px-6 py-3 text-right font-bold text-blue-600">{Math.round((c.placed/c.students)*100)}%</td>
                  ...
                  {data.skillGap.map((item) => (
                    <div key={item.skill}>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className={item.supply < item.demand ? "text-red-500" : "text-green-500"}>{item.supply < item.demand ? "Undersupply" : "Good"}</span>
                        </div>
                        <div className="relative w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div className="absolute top-0 left-0 h-full bg-slate-300 w-[90%] opacity-30"></div> 
                          <div className={`absolute top-0 left-0 h-full rounded-full ${item.supply < item.demand ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${item.supply}%` }}></div>
                        </div>
                    </div>
                  ))}
              </div>
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        ...
        <div className="flex gap-2">
          <input placeholder="Search..." className="text-xs px-3 py-1.5 border rounded-lg w-48" />
          <select className="text-xs border rounded-lg px-2 py-1.5"><option>All Types</option><option>Govt</option><option>Private</option></select>
        </div>
      </div>
      <table className="w-full text-sm text-left">
        <thead className="bg-slate-50 text-slate-500 font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-center">
            ...
            <div className="flex items-center justify-center gap-4">
               <div><span className="text-2xl font-bold text-blue-600">{demo.male}%</span><div className="text-xs">Male</div></div>
               <div className="h-8 w-px bg-slate-200"></div>
               <div><span className="text-2xl font-bold text-pink-500">{demo.female}%</span><div className="text-xs">Female</div></div>
            </div>
         </div>
         <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-center">
            <h4 className="text-slate-500 text-xs uppercase font-bold mb-2">Course Level</h4>
            <div className="flex items-center justify-center gap-4">
               <div><span className="text-2xl font-bold text-indigo-600">{demo.undergrad}%</span><div className="text-xs">Undergrad</div></div>
               <div className="h-8 w-px bg-slate-200"></div>
               <div><span className="text-2xl font-bold text-purple-500">{demo.postgrad}%</span><div className="text-xs">Postgrad</div></div>
            </div>
         </div>
         <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center items-center">
            ...
            <div className="text-3xl font-black text-slate-800">145,000</div>
         </div>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
         <h3 className="font-bold text-slate-900 mb-4">Student Search (National Database)</h3>
         <div className="flex gap-2">
            <input placeholder="Enter Enrollment ID / Name" className="flex-1 border border-slate-300 rounded-lg px-4 py-2" />
            <button className="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold text-sm">Search</button>
         </div>
         <p className="text-xs text-slate-400 mt-2">Access restricted to authorized personnel.</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="h-64"><SimpleLineChart data={trend} /></div>
         </div>
         <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="space-y-3">
               {recruiters.map((r, i) => (
                  <div key={r.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                     <div className="flex items-center gap-3">
                        <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-xs font-bold border border-slate-200">{i+1}</div>
                        <span className="font-semibold text-slate-700">{r.name}</span>
                     </div>
                     <span className="text-sm font-bold text-slate-900">{r.hires.toLocaleString()} Hires</span>
                  </div>
               ))}
            </div>
         </div>
             ...
             <div key={r.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-xl hover:border-blue-300 transition-colors cursor-pointer group">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-xs text-slate-500">Generated: {r.date} • Size: {r.size}</p>
                   </div>
                </div>
                <button className="px-4 py-2 text-sm font-bold text-blue-600 bg-blue-50 rounded-lg group-hover:bg-blue-100 transition-colors">Download</button>
             </div>
          ))}
       </div>
    </div>
